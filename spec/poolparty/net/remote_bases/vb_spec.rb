...
describe "vb remote base" do
  ...
  describe "launching" do
    before(:each) do
      @tr.vb.stub!(:run_instances).and_return true
    end
    it "should call run_instances on the ec2 Base class when asking to launch_new_instance!" do
      @tr.vb.should_receive(:run_instances).and_return true
    ...
    it "should use a specific security group if one is specified" do
      ...
      @tr.vb.should_receive(:run_instances).with(hash_including(:group_id => ['web'])).and_return true
      @tr.launch_new_instance!
    end
    it "should use the default security group if none is specified" do
      @tr.vb.should_receive(:run_instances).with(hash_including(:group_id => ['default'])).and_return true
      @tr.launch_new_instance!
    end
